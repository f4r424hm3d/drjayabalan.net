...
@push('title')
  <title>{{ $page_title }} - Invoice App</title>
...
@section('main-section')
                ...
                <tbody>
                  ...
                  @foreach ($rows as $row)
                    <tr id="row{{ $row->id }}">
                      ...
                      <td>
                        Appointment Date : {{ getFormattedDate($row->appointment_date, 'd M Y') }} <br>
                        Treatment : {{ $row->getTreatment->treatment_name ?? 'N/A' }} <br>
                        <p style="text-align: justify">
                          <q><i>{{ $row->message }}</i></q>
                        </p>
                      </td>
                      <td>
                        {{-- <a href="{{ aurl('client/profile/' . $row->id) }}" type="button"
                          class="btn btn-xs btn-info waves-effect waves-light" target="_blank" data-toggle="tooltip"
                          title="Go to profile"><i data-feather="user"></i></a> --}}
  ...
  <script>
    ...
    function myfunction(id1) {
      if (id1 != '') {
        window.open("{{ aurl('client/?page=') }}" + id1, "_SELF");
      }
    }
    ...
    function ajaxMoveLead(col, val) {
      //alert(id+' '+val);
      var tbl = 'tbl_admin';
      var id = moveToModelId;
      //alert(id+' '+col+' '+val);
      if (id != '') {
        $.ajax({
          url: "{{ aurl() }}Admin/ajaxUpdate",
          method: "POST",
          data: {
            id: id,
            val: val,
    ...
    function ajaxBulkUpdate(col, val) {
      ...
      if (deleteConfirm == true) {
        ...
        $.ajax({
          ...
          success: function(response) {
            if (col == 'called' && val == 1) {
              $('tr.selectedRow span.tempCalled').fadeIn(500);
              $('tr.selectedRow span.permCalled').hide(500);
            } else if (col == 'called' && val == 0) {
              $('tr.selectedRow span.tempCalled').fadeOut(500);
              $('tr.selectedRow span.permCalled').fadeOut(500);
            } else if (col == 'wapp' && val == 1) {
              $('tr.selectedRow a.tempWapp').fadeIn(500);
              $('tr.selectedRow a.permWapp').hide(500);
            } else if (col == 'wapp' && val == 0) {
              $('tr.selectedRow a.tempWapp').fadeOut(500);
              $('tr.selectedRow a.permWapp').fadeOut(500);
            } else {
              location.reload(true);
            }
            //location.reload(true);
            //$('.selectedRow').fadeOut(1500);
          }
        });
      }
    }
